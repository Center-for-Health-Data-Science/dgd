...
class GaussianMixture(nn.Module):
    def __init__(self, Nmix, dim, type='diagonal',
        ...
        # Log inverse standard deviation with shape (Nmix,dim) or (Nmix,1)
        self.sd_init = sd_init
        self.betafactor = dim*0.5
        ...
        if type == 'fixed':
            ...
            #if loginv_var_prior is None:
            #    self.loginv_var_prior = gaussian(math.log(1/var_init[0]),var_init[1])
            #else:
    ...
    def reshape_targets(self, y, y_type='true'):
        ...
        if y_type == 'true':
            ...
            return y.unsqueeze(1).unsqueeze(1).expand(-1,self.new_samples,self.Nmix,-1)
        elif y_type == 'predicted':
            if len(y.shape) > 2:
                raise ValueError('Unexpected shape in input to function reshape_targets. Expected 2 dimensions, got '+str(len(y.shape)))
            n_points = int(torch.numel(y) / (self.new_samples*self.Nmix*y.shape[-1]))
    ...
    def choose_best_representations(self, x, losses):
        ...
        best_sample = torch.argmin(losses.view(-1,self.new_samples*self.Nmix), dim=1).squeeze(-1)
        best_rep = x.view(n_points,self.new_samples*self.Nmix,self.dim)[range(n_points),best_sample]
...
class GaussianMixtureSupervised(GaussianMixture):
    ...
    def forward(self,x,label=None):
        ...
        if 999 in label:
            ...
            y_sup = self.pi_term - (x.unsqueeze(-2).unsqueeze(-2) - self.mean.view(self.Nclass,self.Ncpc,-1)).square().mul(halfbeta.unsqueeze(-2)).sum(-1) + (self.betafactor*self.logbeta.view(self.Nclass,self.Ncpc,-1)).sum(-1)
            ...
            halfbeta = 0.5*torch.exp(self.logbeta)
            # Pick only the Nclc components belonging class
            y = self.pi_term - (x.unsqueeze(-2).unsqueeze(-2) - self.mean.view(self.Nclass,self.Ncpc,-1)).square().mul(halfbeta.unsqueeze(-2)).sum(-1) + (self.betafactor*self.logbeta.view(self.Nclass,self.Ncpc,-1)).sum(-1)
    ...
    def supervised_sampling(self,label, sample_type='random'):
        ...
        #return samples.view(len(label),self.Ncpc*self.Nclass,self.dim)[range(len(label)),label]
